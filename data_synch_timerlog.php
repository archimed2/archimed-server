...
include("config.php");
	...
	for($s = 0; $s < $cnt; $s++) {	
				...
				$txt_TASK_COMMENTS =  mysql_escape_string($TimerLogArr[$s]['oT_TASK_COMMENTS']);
				$txt_SERVER_UID =  $TimerLogArr[$s]['oSERVER_UID'];
				
				$txt_SERVER_ID = $TimerLogArr[$s]['oSERVER_ID'];		//For update the Exist Records
				$txt_oCommentServerId = $TimerLogArr[$s]['oCommentServerId'];
				...
				if($txt_SERVER_ID !=0 && $txt_SERVER_ID != "")	{						
					...
					$ExecQry = "INSERT INTO tbl_timer_log (T_USER_ID,T_ROOM_NUM,T_TASK_ID,T_TASK_DATE,T_TASK_ENDTIME,T_TASK_COMMENTS,T_COMMENT_ID) VALUES (						
						".$txt_SERVER_UID.",
						'".$txt_ROOM_NUM."',
						".$txt_TASK_ID.",
						'".$txt_TASK_DATE."',
						'".$txt_TASK_ENDTIME."',
						'".$txt_TASK_COMMENTS."',".$txt_oCommentServerId."
						)";
						$exec = mysql_query($ExecQry); 

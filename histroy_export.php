...
require_once('Classes/PHPExcel/IOFactory.php');
	...
	$__FloorPointsArray[18] = array("67", "709", "room_tri_birum", html_entity_decode("Kontor Spl.&#230;ge",ENT_COMPAT,"UTF-8"), "57", "20");
	...
	function getTaskNameArrayList(){
		...
		while($rows = mysql_fetch_array($tQry) ){	
			...
			$TaskNameArr[$rows['T_TASK_ID']] = mb_convert_encoding($rows['T_TASK_NAME'],"UTF-8",'iso-8859-1');
		}
		//return $TaskNameArr;
	}
	//print "<pre>";
	//print "<pre>"; print_r($TaskNameArr);
$st_date = $_REQUEST['create_date_archi']." 00:00:00";
$closedate = $_REQUEST['end_date_archi']." 23:59:59";

//$st_date = $closedate =  "2013-09-10";



 //$qry = mysql_query("SELECT cs.*,t.*,u.T_UNAME,T_PROFESSION FROM  tbl_users as u INNER JOIN   tbl_timer_log as t  ON t.T_USER_ID = u.T_USER_ID  LEFT JOIN tbl_user_comments as cs ON  t.T_COMMENT_ID = cs.T_COMMENT_ID  AND  CAST(t.T_TASK_DATE as DATE) BETWEEN '".$st_date."' AND '".$closedate."'");
 
 $qry = mysql_query("SELECT cs.*,t.*,u.T_UNAME,T_PROFESSION FROM  tbl_users as u INNER JOIN   tbl_user_comments as cs ON cs.T_USER_ID = u.T_USER_ID   LEFT JOIN  tbl_timer_log as t   ON  cs.T_COMMENT_ID  = t.T_COMMENT_ID  WHERE  CAST(cs.T_COMMENT_DATE as DATE) BETWEEN '".$st_date."' AND '".$closedate."'");
...
$userProfession = array("0"=>"","1"=>$doctor,"2"=>"Sygeplejerske","3"=>"Patient","4"=>"Anden funktion");
...
$objPHPExcel->getActiveSheet()->setCellValue('F1', "Tidsstempel");
$objPHPExcel->getActiveSheet()->getStyle('F1')->getFont()->setBold(true);

$objPHPExcel->getActiveSheet()->setCellValue('G1', "Opg. Kommentar");
$objPHPExcel->getActiveSheet()->getStyle('G1')->getFont()->setBold(true);

$objPHPExcel->getActiveSheet()->setCellValue('H1', "Profession");
$objPHPExcel->getActiveSheet()->getStyle('H1')->getFont()->setBold(true);

$objPHPExcel->getActiveSheet()->setCellValue('I1', "Kommentar (hel tidstagning)");
$objPHPExcel->getActiveSheet()->getStyle('I1')->getFont()->setBold(true);

$objPHPExcel->getActiveSheet()->setCellValue('J1', "Info ved login");
...
while($arr = mysql_fetch_array($qry)){ 
	//Spilit the Arrays for Generate the XLS out puts
	if($initialComments == $arr['T_COMMENT_ID']) {
		$DataExcelArr[$m] = array(	'T_UNAME'=>$arr['T_UNAME'],
									'T_ROOM_NUM'=>$__FloorPointsArray[$arr['T_ROOM_NUM']][3],
									'T_TASK_ID'=>$arr['T_TASK_ID'],
									'TNAME_T_TASK_ID'=>$TaskNameArr["{$arr['T_TASK_ID']}"],
									'T_TASK_ENDTIME'=>$arr['T_TASK_ENDTIME'],
									'T_TASK_DATE'=>$arr['T_TASK_DATE'],
									'T_TASK_COMMENTS'=>$arr['T_TASK_COMMENTS'],
									'T_PROFESSION'=> $userProfession[$arr['T_PROFESSION']]); //$arr;		
	}else{
		$DataExcelArr[$m] = array(	'T_UNAME'=>$arr['T_UNAME'],
									'T_ROOM_NUM'=>$__FloorPointsArray[$arr['T_ROOM_NUM']][3],
									'T_TASK_ID'=>$arr['T_TASK_ID'],
									...
									'T_TASK_DATE'=>$arr['T_TASK_DATE'],
									'T_TASK_COMMENTS'=>$arr['T_TASK_COMMENTS'],
									'T_PROFESSION'=> $userProfession[$arr['T_PROFESSION']],
									'OVERALL_COMMENTS'=> array($arr['T_COMMENTS'],$arr['T_EXTRA_INFO'])); //$arr; 
		$initialComments = $arr['T_COMMENT_ID'];
...
//print "<pre>"; print_r($DataExcelArr); print "</pre>"; 
//exit;
$CntDataExcelArr = count($DataExcelArr);
$x =2;
for($v = 0; $v < $CntDataExcelArr; $v++) {


	if(isset($DataExcelArr[$v]['OVERALL_COMMENTS'])) {	//For Rowwise switching construct the array			
			$overComments = $DataExcelArr[$v]['OVERALL_COMMENTS'][0];
			$extraInfo = $DataExcelArr[$v]['OVERALL_COMMENTS'][1];
			$objPHPExcel->getActiveSheet()->getStyle('I'.$x)->getFont()->setBold(true);
			$objPHPExcel->getActiveSheet()->getStyle('J'.$x)->getFont()->setBold(true);
			
			$objPHPExcel->getActiveSheet()->setCellValue('I'.$x, $overComments);	
			$objPHPExcel->getActiveSheet()->setCellValue('J'.$x, $extraInfo);
			$x++;
	}	
	$objPHPExcel->getActiveSheet()->setCellValue('A'.$x, $DataExcelArr[$v]['T_UNAME']);
	$objPHPExcel->getActiveSheet()->setCellValue('B'.$x, $DataExcelArr[$v]['T_ROOM_NUM']);
	$objPHPExcel->getActiveSheet()->setCellValue('C'.$x, $DataExcelArr[$v]['T_TASK_ID']);
	$objPHPExcel->getActiveSheet()->setCellValue('D'.$x, html_entity_decode($DataExcelArr[$v]['TNAME_T_TASK_ID'],ENT_COMPAT,"UTF-8"));
	$objPHPExcel->getActiveSheet()->setCellValue('E'.$x, $DataExcelArr[$v]['T_TASK_ENDTIME']);
	$objPHPExcel->getActiveSheet()->setCellValue('F'.$x, $DataExcelArr[$v]['T_TASK_DATE']);
	$objPHPExcel->getActiveSheet()->setCellValue('G'.$x, $DataExcelArr[$v]['T_TASK_COMMENTS']);	
	$objPHPExcel->getActiveSheet()->setCellValue('H'.$x, $DataExcelArr[$v]['T_PROFESSION']);
	$x++;
}
